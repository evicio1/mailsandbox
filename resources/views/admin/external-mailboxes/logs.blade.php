<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            <a href="{{ route('admin.external-mailboxes.index') }}" class="text-slate-400 hover:text-white transition">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            </a>
            <div>
                <h1 class="page-title">Sync Logs</h1>
                <p class="page-subtitle">{{ $externalMailbox->email }} ({{ $externalMailbox->tenant->name ?? 'Unknown' }})</p>
            </div>
        </div>
    </x-slot>
